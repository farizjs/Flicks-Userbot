...
async def corona(event):
    ...
    if country_data:
        ...
        output_text += f"`🧪Total tes : {country_data['total_tests']}`\n\n"
        output_text += f"Berdasarkan data [Worldometer](https://www.worldometers.info/coronavirus/country/{country})"
    else:
...
CMD_HELP.update({"covid": "`.covid `**<country>**"
                 "\n`Usage: Dapatkan informasi tentang data covid-19 di suatu negara.`\n\n"
                 "`.covid`"
                 "\n`Usage: Dapatkan informasi tentang data covid-19 di Worldwide.`\n"})
