    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                ...
                ' untuk dapat deploy perubahan terbaru dari ♨️KEN-UBOT♨️.`'
            )
            repo.__del__()
            return
        ...
        await event.edit('`[HEROKU]:'
                         '\nSedang MengUpdate ♨️KEN-UBOT♨️, Mohon Menunggu 5-7 Menit`'
                         )
        ups_rem.fetch(ac_br)
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('**♨️KEN-UBOT♨️** `Berhasil Di Update!`')
    await asyncio.sleep(1)
    await event.edit('**♨️KEN-UBOT♨️** `Di Restart....`')
    await asyncio.sleep(1)
...
async def upstream(event):
    ...
    if changelog == '' and force_update is False:
        await event.edit(
            f'\n♨️KEN-UBOT♨️ Sudah Versi Terbaru\n')
        await asyncio.sleep(15)
        await event.delete()
        return repo.__del__()

    if conf is None and force_update is False:
        changelog_str = f'**Pembaruan Untuk ♨️KEN-UBOT♨️ :\n\n Pembaruan Data :**\n`{changelog}`'
        ...
        return await event.respond('**Perintah Untuk Update, Sebagai Berikut.**\n 𝘾𝙤𝙢𝙢𝙖𝙣𝙙: >`.update now`\n 𝘾𝙤𝙢𝙢𝙖𝙣𝙙: >`.update deploy`\n\n__Untuk Meng Update Fitur ♨️KEN-UBOT♨️.__')

    if force_update:
        await event.edit(
            '`Sinkronisasi Paksa Ke Kode Userbot Stabil Terbaru, Harap Tunggu .....`')
    else:
        await event.edit('` Proses Update ♨️KEN-UBOT♨️, Loading....1%`')
        await event.edit('` Proses Update ♨️KEN-UBOT♨️, Loading....20%`')
        await event.edit('` Proses Update ♨️KEN-UBOT♨️, Loading....35%`')
        await event.edit('` Proses Update ♨️KEN-UBOT♨️, Loading....77%`')
        await event.edit('` Proses Update ♨️KEN-UBOT♨️, Updating...90%`')
        await event.edit('` Proses Update ♨️KEN-UBOT♨️, Mohon Tunggu Sebentar....100%`')
    if conf == "now":
