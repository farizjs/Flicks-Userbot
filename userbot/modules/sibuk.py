...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**Maaf Master {ALIVE_NAME} Sedang SIBUK**\n╰╼═════════╾",
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**{ALIVE_NAME} Sedang SIBUK**\n**Tunggu Sampai Online Kembali Ya**\n╰╼═════════╾",
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n** Master {ALIVE_NAME} Sedang SIBUK**\n**Tunggulah Sampai Online Ya**\n╰╼═════════╾",
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**{ALIVE_NAME} Sedang SIBUK**\n╰╼═════════╾",
]
...
async def set_afk(afk_e):
    ...
    if string:
        ...
        await afk_e.edit(f"**🔥 Sibuk**\n\n╭╼══════════════╾\n**{ALIVE_NAME} Telah Sibuk!**\n╰╼═════════╾")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "✘ sɪʙᴜᴋ ✘"))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="✘ sɪʙᴜᴋ ✘"))
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("✘ sɪʙᴜᴋ ✘"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond(f"🔥 __**MASTER KEMBALI ONLINE**__")
        time.sleep(7)
        await msg.delete()
        ...
        if BOTLOG:
            await notafk.client.send_message(
                ...
                str(len(USERS)) + " Obrolan Saat Anda SIBUK",
            )
            for i in USERS:
...
CMD_HELP.update(
    {
        "sibuk": "** Plugin : **`sibuk`\
        \n\n  •  **Perintah :** `.sibuk`\
        \n  •  **Function : **Memberi tahu kalau Anda sedang SIBUK , dan menguubah nama belakang menjadi ✘ sɪʙᴜᴋ ✘\
