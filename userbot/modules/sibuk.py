...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"__🔥 SIBUK__\n\n╭╼══════════════╾\n**Maaf Bos {ALIVE_NAME} Sedang SIBUK**\n╰╼═════════╾",
...
async def set_afk(afk_e):
    ...
    if string:
        ...
        await afk_e.edit(f"**🔥 Sibuk**\n\n╭╼══════════════╾\n**{ALIVE_NAME} Telah Sibuk!**\n╰╼═════════╾")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "『 𝐒𝐢𝐛𝐮𝐤 』"))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="『 𝐒𝐢𝐛𝐮𝐤 』"))
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("『 𝐒𝐢𝐛𝐮𝐤 』"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond(f"🔥 __**BOS KEMBALI ONLINE**__")
        time.sleep(7)
        await msg.delete()
...
CMD_HELP.update(
    {
        "sibuk": "** 『Plugin』 : **`sibuk`\
        \n\n  •  **Perintah :** `.sibuk`\
        \n  •  **Function : **Memberi tahu kalau Anda sedang SIBUK , dan menguubah nama belakang menjadi 『 𝐒𝐢𝐛𝐮𝐤 』\
