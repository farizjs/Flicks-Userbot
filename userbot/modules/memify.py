...
async def draw_meme_text(image_path, text):
    ...
    m_font = ImageFont.truetype(
        "userbot/utils/styles/kfonts.otf", int(
            (70 / 640) * i_width)
    )
    if ";" in text:
...
async def draw_meme_text(image_path, text):
    ...
    i_width, i_height = img.size
    m_font = ImageFont.truetype(
        "userbot/utils/styles/default.ttf", int((95 / 730) * i_width)
    )
    if ";" in text:
        upper_text, lower_text = text.split(";")
    else:
...
@register(outgoing=True, pattern=r"^\.mmf3(?: |$)(.*)")
async def mim(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
...
async def draw_meme_text(image_path, text):
    ...
    i_width, i_height = img.size
    m_font = ImageFont.truetype(
        "userbot/utils/styles/fonts.otf", int(
            (70 / 640) * i_width)
    )
    if ";" in text:
        upper_text, lower_text = text.split(";")
