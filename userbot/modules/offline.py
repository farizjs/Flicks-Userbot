...
AFKSTR = [
    ...
    f"**Maaf {ALIVE_NAME} Sedang OFF\n Tunggu Sampai Dia Online!**",
    f"**Sorry {ALIVE_NAME} Sedang OFF\n Tunggu Sampe Dia Online!**",
    f"**Maaf {ALIVE_NAME} Sedang OFF!**",
]
...
async def set_afk(afk_e):
    ...
    if string:
        ...
        await afk_e.edit(f"**{ALIVE_NAME} Sedang OFF**")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "✘ ᴏꜰꜰʟɪɴᴇ ✘"))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="✘ ᴏꜰꜰʟɪɴᴇ​ ✘"))
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("✘ ᴏꜰꜰʟɪɴᴇ ✘"):
        last1 = last[:-12]
    else:
        last1 = ""
