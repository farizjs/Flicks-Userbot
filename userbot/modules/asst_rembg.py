...
async def is_register_admin(chat, user):
    if isinstance(chat, (types.InputPeerChannel, types.InputChannel)):
        return isinstance(
            (
                await tgbot(functions.channels.GetParticipantRequest(chat, user))
            ).participant,
...
async def _(event):
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "flicks_rmbg.png"
        ...
        await event.reply("Latar Belakang Dihapus dalam {} detik".format(ms))
    else:
        await event.reply(
            "remove.bg API mengembalikan Kesalahan.\n`{}`".format(
